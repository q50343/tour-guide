<template>
  <div class="detail" v-if="loading === 1">
    <div class="detail-title df-between">
      ...
      <div>
        <button class="detail-btn" @click="window.print()">
          <i class="ico-print"></i>
        </button>
      </div>
    </div>
    ...
    <div class="space"></div>
    <div class="near">
      <div v-for="item in Object.keys(modeLibs)" :key="item">
        <div class="df-between">
          <h2 class="fz-md"><i class="icofont-beach"></i> 查看鄰近的{{modeLibs[item]}}</h2>
...
export default {
  ...
  setup(props) {
    ...
    return { loading, hasHistory, result, modeLibs };
  },
};
</script>
